// Push the recomputed in-air windows.
import fs from "fs";
...
const air = JSON.parse(fs.readFileSync("out/airwindows.json", "utf8"));
const byKey = new Map(air.map((a) => [a.video + "#" + a.n, a]));
let page = 1, all = [];
while (true) {
...
for (const r of all) {
  ...
  await fetch(`${PB}/api/collections/vision_shots/records/${r.id}`, {
    ...
    body: JSON.stringify({ airStart: a.airStart, airEnd: a.airEnd,
                           wideAirStart: a.wideAirStart, wideAirEnd: a.wideAirEnd }),
  });
  n++;
}
console.log(`in-air window updated on ${n} records`);
